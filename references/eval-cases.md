...
- 名称：用户选择现在聊后要求补足背景
- 维度：Process, Style, Evidence
- 用户请求：`现在聊 CAND-20260815-ab12cd34。`
- 已知 Candidate：存在可定位的 before、人的 Deny/Guide、after；证据仅为 Session 与设计文档，没有真实集成结果。
- 预期行为：首次回复先给“访谈启动上下文卡”，依次说明当时的情况、你做的取舍、后来变成什么、这次要核对什么；卡片后只问一个会改变经历含义的问题。
- 确定性检查：输出含四个标准标签 `当时的情况/你做的取舍/后来变成什么/这次要核对什么`；只有一个问号/问题；不要求用户重复 Candidate 已有事实；没有工程评审术语、证据免责声明或对立转折模板；出现 `我` 或 `你` 时分别使用 `我（Agent）` 或 `你（人类）`。
- 语义判定 rubric：用户无需再问“这件事的背景是什么”即可理解访谈对象 1 分；人类判断与 Agent 贡献未被混淆 1 分；未把设计或 Session 说成真实集成 1 分。通过线 3/3。
- 失败迹象：只用一句“当前理解”概括；先抛问题后补背景；完整复述无关 Session；把未验证实施写成结果；用“核心边界”一类工程语言提问。
...
- 名称：用户在采访中说明自己的判断和已经发生的行动
- 维度：Outcome, Process, Trust
- 用户请求：`当时我判断这类问题不能只修一个 Bug，所以让 Agent 把模式固化进 Harness；后来我们也做了。`
- 预期行为：标为 `human_reported`，按用户原意记录判断、行动与结果；需要澄清时只问一个中性问题。除非用户另行明确要求核验，否则不搜索代码、测试、文档、历史 Session 或外部资料，不评价陈述可信度，也不追加证据上限或“尚不能证明已实施”等免责声明。
- 确定性检查：采访回复、Confirmation Brief、Episode 正文和完成提示均未新增用户未表达的动机、因果、价值判断、技术状态或结果；没有因采访回答触发额外工程搜索；`human_reported` 仅作为来源标记。
- 语义判定 rubric：用户陈述被忠实保留 1 分；采访与工程核验职责分离 1 分；没有审计式免责声明 1 分。通过线 3/3。
- 失败迹象：先去 Git 找 commit 再决定是否接受回答；把 `human_reported` 解释为低可信；写“证据上限为 design”“尚不能证明规则已实施”；根据工程材料反向改写用户原话。
