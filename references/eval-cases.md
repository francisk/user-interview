...
- 预期行为：首次回复先给“访谈启动上下文卡”，依次说明当时的情况、你做的取舍、后来变成什么、现在还不能说什么；卡片后只问一个会改变经历含义的问题。
- 确定性检查：输出含四个标准标签 `当时的情况/你做的取舍/后来变成什么/现在还不能说什么`；只有一个问号/问题；不要求用户重复 Candidate 已有事实；没有工程评审术语或对立转折模板；出现 `我` 或 `你` 时分别使用 `我（Agent）` 或 `你（人类）`。
