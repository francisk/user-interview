...
| 评估层 | 22 个行为用例与 21 个触发用例；自动校验目录结构，模型行为需单独评估 | `eval-cases.md`、`trigger-tests.csv`、`scripts/validate_eval_catalog.py` |
...
4. Eval 回查：路由优先级、Session 澄清、单文档边界、兼容旧 weekly、双确认、六种可选心境路径（含跨 Session 已知先前邀请与历史不足）及无本机 Python 路径均有用例；目录结构由程序校验，模型执行结果不由这些单元测试代替。
5. 效用检查：11 类故障覆盖表、失败响应、显式高风险黑名单、确定性脚本与负向测试齐全；心境成为完成门槛、跳过后重复探问、由语气/行为推断、追加评价或免责声明、或以占位符持久化任一项均不通过质量审查。

## 成熟度

- 当前等级：L4。
- 依据：有明确触发和路由、稳定契约、确定性脚本、失败保护、安全边界、行为/触发评估目录和兼容回归。行为用例是否通过必须引用一次真实目标 Agent 评估，不能由目录校验推断。
